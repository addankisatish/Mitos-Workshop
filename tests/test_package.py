...
class PackageTests(unittest.TestCase):
    """Exercise the public import and the intentionally quiet Day 4 CLI stub."""
    ...
    def test_cli_stub_and_module_execution_succeed(self) -> None:
        """Both direct and ``python -m mitos`` entry paths return successfully."""
        self.assertIsNone(main())
        completed = subprocess.run(
            [sys.executable, "-m", "mitos"], capture_output=True, text=True, timeout=10
        )
